...
\begin{comment}
 ...
 StV: was this used for the 8-bit redigitization pipeline for XXYY data? \\
 VK: No, as far as I'm aware. This attribute is not set in the digitize.py.
\end{comment}
\begin{comment}
...
\begin{comment}
 StV: Re POSITION. I would do the same as the TBUF Antenna Field Group and add unit, frame and epoch explicitly. \\
 VK: I would agree about POSITION\_EPOCH, but is there a reason to believe that POSITION\_UNITS here can be anything else than ``meters``, and for the POSITION\_FRAME it can be anything else than ``ITRF``? I will add POSITION\_EPOCH for now and lets discuss about other two.
\end{comment}
\begin{comment}
 StV: Re TIME\_FAILURE. is this an array of failure times or the first time it failed? \\
 VK: this is the array of all times of failures for all elements in the antenna field. Both arrays TIME\_FAILURE and ELEMENT\_FAILURE should be used together to know to what element the failure corresponds to. If the same elements failed 10 times during the observations, the ELEMENT\_FAILURE array will have 10 same integer values which is the ELEMENT\_ID in the ELEMENT\_OFFSET array. The TIME\_FAILURE will have 10 times when these failures occurred.
\end{comment}
...
\bottomcaption{The Antenna Field Group Header Attributes
  ...
  \begin{supertabular}{|lllp{5.0cm}|}
    ...
    \verb|POSITION_EPOCH| & \verb|string| & \verb|'2026.5'| & Epoch of the reference position frame (same as \verb|ANTENNA_FIELD_POSITION_EPOCH| for Tbuf data) \\
...
\begin{comment}
 StV: You prefer to have this information separate, rather than as a grouping between SAP and BEAM? \\
 VK: yes, because the BF pipeline is for the whole SAP (not TAB) or even a set of SAPs, so in case of 100+ TABs the same info would be duplicated between all TABs, not efficient. I agree that currently there are settings in the Beam group that could be removed when we introduce this Group (e.g. CHANNELS\_PER\_SUBBAND, STATIONS\_LIST, etc), but I would like to keep them as they are for backwords compatibility.
\end{comment}
\begin{comment}
 VK: I think we should also add BANDWIDTH (and \_UNIT) and FREQUENCY\_CENTER (and \_UNIT) attributes just to quickly get the values, instead of calculating based on subbands
\end{comment}
...
\begin{small}
  ...
  \begin{tabular}{|lllp{5.0cm}|}
    ...
    \verb|BANDWIDTH| & \verb|double| & --- & Bandwidth related to subbands used, total width of all subbands of this beam \\
    \verb|BANDWIDTH_UNIT|  & \verb|string| & \verb|`MHz'| &  \\
    \small{\verb|FREQUENCY_CENTER|}  & \verb|double| & --- & Center frequency - the middle frequency of used subbands' range \\
    \small{\verb|FREQUENCY_CENTER_UNIT|}  & \verb|string| & \verb|`MHz'| & \\
...
\begin{comment}
 ...
 StV: we can put the JSON specification for COBALT in, if we move to that. The tricky thing is how to deal with COBALT overrides. Keywords that you need to interpret the data should just be in the metadata. delayCompensation and correctClocks are important to know if you can reprocess fly's eye data with Cobalt. \\
 VK: Parset override files should be already available before the observation, so this override file can be added to HDF5 metadata already at the time of creation.
\end{comment}
\begin{comment}
...
\begin{comment}
 ...
 StV: We need only one POINT\_DIRECTION\_TYPE or TRACKING. Tbuf data uses BEAM\_DIRECTION (with \_UNIT and \_FRAME). If we used POINT\_DIRECTION\_TYPE before I would just use that and remove TRACKING. \\
 VK: I think neither TRACKING nor POINT\_DIRECTION\_TYPE were used before
\end{comment}
\begin{comment}
...
\begin{comment}
 ...
 - BARYCENTERED \\
 StV: For Central COBALT I think you are correct. We can re-add them if at some point this will be implemented. But do check with the EuroFlash team as the dataformat could potentially be used for data snippets for which this would be interesting.
\end{comment}
\begin{comment}
 VK: I think we should also add BEAM\_BANDWIDTH (and \_UNIT) as number of subbands in a given SAP (and thus BEAM) can be different from the total in Observation. We already have BEAM\_FREQUENCY\_CENTER, so it's logical to add BANDWIDTH as well.
\end{comment}
...
\begin{supertabular}{|lllp{5.5cm}|}
  ...
  \small{\textcolor{blue}{\texttt{BEAM\_BANDWIDTH}}}  & \verb|double| & --- & Beam bandwidth, total width of all subbands of this beam \\
  \small{\textcolor{blue}{\texttt{BEAM\_BANDWIDTH\_UNIT}}}  & \verb|string| & \verb|`MHz'| &  \\
