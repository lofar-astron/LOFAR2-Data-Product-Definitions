...
%  \item LOFAR 2.0 world coordinate conventions (for antenna locations)
  ...
  \item Old H5Parm specifications: \href{https://docs.google.com/document/d/1pjDxRywhRO5HRuqciGYO2TWD-Ufu_5nxwFA3b_YnJJg/edit?tab=t.0#heading=h.uae9zka4fdfd}{HDF5 solutions tables for LOFAR (H5parm)}, v1.0 (26/07/13).
\end{itemize}
...
This document is structured as follows: In \autoref{sec:organization}, we introduce conventions and the structure of the data format, in \autoref{sec:details}, we provide details on the structure, and in \autoref{sec:interfaces}, we discuss the requirements for interfaces related to the h5parm format.

\section{Organization of the Data}\label{sec:organization}
...
\begin{center}
  \begin{tabular}{rp{11cm}}
    ...
    weight & Values between $0$ and $1$ used when applying calibration solutions. A weight of $0$ results in the corresponding data being flagged upon application of the solutions.\\
...
H5parms are designed such that solution value arrays are stored as chunked arrays (CArray-equivalent) suitable for
...
\section{Detailed Data Specification}\label{sec:details}
...
\begin{itemize}
  \item One or more solset groups exist (e.g. 'sol000', or 'calibrator'), each representing a distinct dataset/context.
\end{itemize}
...
\subsubsection{Metadata attributes: \texttt{<solset>.attrs}}
The solset contains required metadata attributes, which are either commong LOFAR attributes or h5parm solution-specific attributes.

\begin{verse}
\verb|Metadata Attributes = Common LOFAR Attributes + H5parm specific Attributes.|
\end{verse}


\subsubsection*{Common LOFAR Attributes: \texttt{<solset>.attrs}}
The Common LOFAR Attributes are the first attributes of any solset attributes. They are common to all LOFAR science data products.  
Table~\ref{tab:lofar common metadata} lists the Common LOFAR Attributes which all also apply to the h5parm solution data definition. These attributes are required to be attributes of the solset group: \texttt{<solset>.attrs}; if a value is not available for an Attribute, a \verb|`NULL'| maybe used in its place.
There are scenarios where multiple LOFAR observations map to a single h5parm file (e.g., deriving a bandpass, combining solutions across multiple epochs). In these cases, observation-level common attributes can be mapped to lists.
\input common_tables/lofar_common_metadata

\subsubsection*{Additional h5parm solution Root Attributes: \texttt{<solset>.attrs}}
The h5parm comes with the additional Root Attributes:
\begin{itemize}
  \item Type: bytestring
  \item Required attributes:
  \begin{itemize}
    \item \texttt{<solset>.attrs["h5parm\_version"]}: Version of the h5parm solution format, e.g. `1.0'
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Attribute \texttt{type}: a string describing solution type. While this is in principle flexible, the following strings are used for standard solution types: \texttt{phase}, \texttt{amplitude}, \texttt{clock}, \texttt{tec}, \texttt{rotationmeasure}.
  \item Optional attribute \texttt{default}: scalar default value for missing data (typically 0 for phase-, tec-, clock- or rotation-like, 1 for scalar or diagonal-amplitude-like, diag(1,1) for fulljones), as used in examples. 
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{dir}: string (16 char); direction/source identifiers (as listed in table \texttt{<solset>/source}). 
  \item \texttt{pol}: string (2 char); polarization identifiers (standard cases are \texttt{[I]} for scalar, \texttt{[XX,YY]} for linear-basis diagonal, \texttt{[RR,LL]} for circular basis diagonal, \texttt{[XX,XY,YX,YY]} for linear-basis full-Jones, \texttt{[RR,RL,LR,LL]} for circular-basis full-Jones). \end{itemize}
...
\begin{itemize}
  ...
  \item Element type: float64 
  \item Shape: \texttt{(N\_axis1, N\_axis2, ...)} following the axis order defined below
  \item Units: solution values do not explicitly record the unit, the following standard units are used for different \texttt{types}: dimensionless (\texttt{amplitude}), rad (\texttt{phase, rotation}), total electron content units / TECU (\texttt{tec}), rad/m$^2$ (\texttt{rotationmeasure}).
  \item Required attributes:
  \begin{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item continuous values in \texttt{[0,1]} are used to represent data weights. 
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item The h5parm version currently is "1.0" (pre-LOFAR2), we might want to bump up that number to 1.1 and/or sync it to version of this DDD once the LOFAR2 changes are introduced.
...
\begin{itemize}
  ...
  \item Define recommended compression and chunking settings for typical LOFAR 2.0 solution volumes.
  \item h5repack to reduce file size
\end{itemize}
