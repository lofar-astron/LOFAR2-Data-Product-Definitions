...
\title{LOFAR Data Definition Document 009: \\ Calibration Solution Files (H5Parm) \\
{\normalsize Document ID: LOFAR2-DDD-009-H5parm} \\
{\normalsize Revision 0.1} \\
{\normalsize SVN Repository Revision: \svnInfoRevision} \\
\author{LOFAR 2.0 Data Working Group (DWG)}
...
\subsection*{R1: File format}
Calibration solution files shall be stored as \textbf{HDF5} files following the
\textbf{H5Parm} conventions described in the H5Parm specification.

\subsection*{R2: Support multiple datasets}
A single file shall support multiple datasets by containing multiple solution-sets (solsets),
e.g. one solset for a calibrator and one for a target. 

\subsection*{R3: Support arbitrary solution types}
A solset shall support an arbitrary number of solution tables (soltabs), including standard
solution types such as amplitude, phase, rotation, rotationmeasure, clock, TEC, and customized user-defined
types. 

\subsection*{R4: Support flexible axes}
Each soltab shall define its own axis set and axis ordering, via an attribute on the values
(and weight) arrays. 

\subsection*{R5: Efficient access and large volume support}
Solution value arrays shall be stored as chunked arrays (CArray-equivalent) suitable for
efficient slicing without loading full arrays into memory. 

\subsection*{R6: Flagging and missing data}
The format shall support missing samples and flagging:
\begin{itemize}
  \item Missing samples may be stored as NaN in values. All NaN values shall be flagged (have weights equal to zero). 
  \item Weights shall encode flagged solutions using weight=0 (and may use continuous values in [0,1]). 
\end{itemize}
...
\begin{itemize}
  ...
  \end{itemize}
  \item Missing data: may be stored as NaN (\emph{holes}).
\end{itemize}
