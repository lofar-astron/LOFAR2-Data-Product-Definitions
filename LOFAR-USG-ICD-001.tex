...
\begin{table}[htbp]
  ...
  \begin{tabular}{|llllp{4.5cm}|}
    ...
    \small {\texttt{CLOCK\_SOURCE}} & Attr & \verb|string| & DDD v1.0 & The origin of the clock. White Rabbit + source of clock or GPS\\
    \small {\texttt{ANTENNA\_FIELD\_XBAX}} & Group & --- & DAL v4.0 & Container group for dipole datasets per antenna field; The name is either \texttt{ANTENNA\_FIELD\_LBA} for the low band antennas or \texttt{ANTENNA\_FIELD\_HBA}, \texttt{ANTENNA\_FIELD\_HBA0}, or \texttt{ANTENNA\_FIELD\_HBA1} for the high band antennas. \\
    \hline
  \end{tabular}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|llllp{4.5cm}|}
    ...
    \small {\texttt{ANTENNA\_FIELD\_POSITION}} & Attr & \verb|array<double,1>| & DAL v4.0 & [3] Numerical value of the station position coordinates of this antenna field. \\
    \small {\texttt{ANTENNA\_FIELD\_POSITION\_UNIT}}  & Attr & \verb|string| & DAL v4.0 & Physical units associated with the numerical values for the station position. \\
    \small {\texttt{ANTENNA\_FIELD\_POSITION\_FRAME}} & Attr & \verb|string| & DAL v4.0 & Identifier for the reference frame within which the station position is provided. \\
    \small {\texttt{BEAM\_DIRECTION}} & Attr & \verb|array<double,1>| & DAL v4.0 & [2] Numercial value of the analog beamformer direction for this antenna field. The analog beamformer is used for HBA and NenuFar.\\
    \small {\texttt{BEAM\_DIRECTION\_UNIT}}  & Attr & \verb|string| & DAL v4.0 & Physical units associated with the numerical value of the beam direction. Mandatory for \verb|HBA| station data.\\
    \small {\texttt{BEAM\_DIRECTION\_FRAME}} & Attr & \verb|string| & DAL v4.0 & Identifier for the reference frame within which the beam direction is provided. Mandatory for \verb|HBA| station data.\\
    ...
    \small\verb|ANTENNA_SET| & \small\verb|string| & ---  & DDD v1.0.0 & Antenna set specification (see Table~\ref{tab:antenna-set}) \\
    \small \verb|FILTER_SELECTION| & \small\verb|string| & ---  & DDD v1.0.0 & Filter selection (see Table~\ref{tab:filter-selection}) \\
    ...
    \small {\texttt{DIPOLE\_\{SSSSSFFFAAP\}\}}} & Dataset & \verb|array<short,1>| & DAL v4.0 & Dataset containing the actual raw samples read out from the transient buffer; the name of the dataset is constructed from the \small \textit{\texttt{STATION\_NAME}} (SSSSS), \verb|ANTENNA_TYPE| (FFF) (LBA or HBA), \verb|ANTENNA_ID| (AA) and \verb|POLATIZATION| (P) (X or Y). \\
  \end{tabular}
...
\begin{center}
  ...
  \begin{supertabular}{|lllp{4.25cm}|}
    ...
    \small {\texttt{STATION\_NAME}} & \texttt{unsigned} & DAL v2.0 & Data source station identifier. \\
    \small {\texttt{ANTENNA\_ID}} & \verb|unsigned| & DAL v2.0 & Antenna identifier. \\
    \small {\texttt{POLARIZATION}} & \verb|string| & DDD v1.0 & Polarization of this dipole. X or Y.\\
    ...
    \small \textit{\texttt{TILE\_BEAM}} & \verb|array<double,1>| & DDD v1.0 & Numerical value of the tile beam direction. \\
    \small \textit{\texttt{TILE\_BEAM\_UNIT}} & \verb|string| & DAL v2.0 & Physical units of the tile beam orientation. \\
    \small \textit{\texttt{TILE\_BEAM\_FRAME}} & \texttt{string} & DDD v1.0 & Reference frame of the tile beam.  \\
    \small \textit{\texttt{TILE\_ANTENNAS\_ENABLED}} & \texttt{array<bool,16>} & DDD v1.0 & Antennas within the tile on or off.  \\
    \small \textit{\texttt{TILE\_ANTENNA\_POSITION\_OFFSET}} & \texttt{array<bool,16>} & DDD v1.0 & Antenna offset relative  to the tile center and in the same frame as the tile center as given by \texttt{ANTENNA\_POSITION}.  \\
    \small \textit{\texttt{TILE\_ANTENNA\_POSITION\_OFFSET\_UNIT}} & \texttt{array<bool,16>} & DDD v1.0 & Unit of the tile dipoles position offset.  \\
    \small \textit{\texttt{TILE\_BEAMFORMER\_DELAYS}} & \texttt{array<float,16>} & DDD v1.0 & Delays for the different antennas in the tile beamformer.   \\
    \small \textit{\texttt{TILE\_BEAMFORMER\_DELAYS\_UNIT}} & \texttt{string} & DAL v4.0 & Unit of tile beamformer delays. Default nanosecond.  \\
    \small \textit{\texttt{ADC2VOLTAGE}} & \verb|double| & DAL v2.0 & Value for converting ADC value to voltages. \\
    \hline
  \end{supertabular}
